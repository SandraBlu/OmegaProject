...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnMarkerAdded, class UNavMarkerComponent*, Marker);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnMarkerRemoved, class UNavMarkerComponent*, Marker);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnDiscoverPointOfInterest, class APOI*, POI);
...
class OMEGAPROJECT_API UNavigationComponent : public UActorComponent
{
	GENERATED_BODY()

protected:
	friend class UNavMarkerComponent;
	...
	virtual bool AddMarker(class UNavMarkerComponent* NavMarker);
	virtual bool RemoveMarker(class UNavMarkerComponent* NavMarker);

	UFUNCTION(Server, Reliable)
	void ServerSelectMarker(class UNavMarkerComponent* Marker);

	UFUNCTION(BlueprintCallable, Category = "Navigation")
	void SelectMarker(class UNavMarkerComponent* Marker);
	...
	* 
	...
	virtual bool Save(const FString& SaveName = "NavigatorSaveData", const int32 Slot = 0);

	/**Load custom waypoints back in from disk
	@param SaveName the name of the save game. */
	UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly, Category = "Saving")
	virtual bool Load(const FString& SaveName = "NavigatorSaveData", const int32 Slot = 0);
...
public:
	...
	virtual bool HasDiscoveredPOI(class APOI* POI) const;

	virtual void HandleEnterPOI(class APOI* POI);

	UFUNCTION(Client, Reliable)
	virtual void ClientSetPOIDiscovered(class APOI* POI);

	virtual void SetPOIDiscovered(class APOI* POI);
	...
	TArray<class UNavMarkerComponent*> Markers;
