...
struct FItemStat
{
	...
	FItemStat(const FText& InStatDisplayName, const FString& InStringVariable) : StatDisplayName(InStatDisplayName), StringVariable(InStringVariable)
	{
	...
	FItemStat()
	{
		StatDisplayName = NSLOCTEXT("ItemStat", "StatDisplayName", "Stat Display Name");
...
class OMEGAPROJECT_API UOItem : public UObject
{
	GENERATED_BODY()
...
protected:
	...
	*/
	...
	class UOInventoryComponent* OwningInventory;
	...
	virtual void AddedToInventory(class UOInventoryComponent* Inventory);
	virtual void RemovedFromInventory(class UOInventoryComponent* Inventory);
