...
#include "OInventoryComponent.generated.h"
...
public:
	...
	TSubclassOf<class UOItem> ItemClass = TSubclassOf<class UOItem>();

	//Any new stacks that were created
	UPROPERTY(BlueprintReadOnly, Category = "Item Add Result")
	TArray<class UOItem*> Stacks = {};
	...
	static FItemAddResult AddedSome(TArray<class UOItem*> InItems, const int32 InItemQuantity, const int32 ActualAmountGiven, const FText& ErrorText)
	{
	...
	static FItemAddResult AddedAll(TArray<class UOItem*> InItems,const int32 InItemQuantity)
	{
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnItemRemoved, class UOItem*, Item, const int32, Amount);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnBeginLooting, class UOInventoryComponent*, LootSource);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnEndLooting, class UOInventoryComponent*, LootSource);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnItemUsed, class UOItem*, Item);
...
public:
	...
	TSoftClassPtr<class UOItem> Item;
 ...
 */
UCLASS(ClassGroup=(OmegaProject), DisplayName = "Inventory", meta=(BlueprintSpawnableComponent))
class OMEGAPROJECT_API UItemCollection : public UDataAsset
{
	GENERATED_BODY()
...
public:	
	...
	UOInventoryComponent(const FObjectInitializer& ObjectInitializer);

UFUNCTION(BlueprintCallable, Category = "Inventory")
	virtual bool UseItem(class UOItem* Item);
	...
	virtual FItemAddResult TryAddItemFromClass(TSubclassOf<class UOItem> ItemClass, const int32 Quantity = 1, const bool bCheckAutoUse=true);
	...
	UFUNCTION(BlueprintCallable, Category = "Inventory")
	virtual int32 ConsumeItem(class UOItem* Item, const int32 Quantity = 1);
	...
	UFUNCTION(BlueprintCallable, Category = "Inventory")
	virtual bool RemoveItem(class UOItem* Item);
	...
	virtual bool HasItem(TSoftClassPtr<class UOItem> ItemClass, const int32 Quantity = 1, const bool bCheckVisibility = false) const;

	/**Return true if the taker can loot the item from this inventory */
	UFUNCTION(BlueprintPure, Category = "Inventory")
	virtual bool AllowLootItem(class UOInventoryComponent* Taker, TSubclassOf<class UOItem> ItemClass, const int32 Quantity, FText& ErrorText) const;

	/**Return true if the storer can store the given item in this inventory */
	UFUNCTION(BlueprintPure, Category = "Inventory")
	virtual bool AllowStoreItem(class UOInventoryComponent* Storer, TSubclassOf<class UOItem> ItemClass, const int32 Quantity, FText& ErrorText) const;

	/** Remove the item from us, and give it to the taker. Return how much of the item was able to be looted */
	virtual FItemAddResult PerformLootItem(class UOInventoryComponent* Taker, TSubclassOf <class UOItem> ItemClass, const int32 Quantity = 1);

	/** Remove the item from the storer, and give it to us. Return how much of the item was able to be stored*/
	virtual FItemAddResult PerformStoreItem(class UOInventoryComponent* Storer, TSubclassOf <class UOItem> ItemClass, const int32 Quantity = 1);

	/**Return the first item with the same class as ItemClass.*/
	UFUNCTION(BlueprintPure, Category = "Inventory")
	UOItem* FindItemByClass(TSoftClassPtr<class UOItem> ItemClass, const bool bCheckVisibility = false) const;

	/**Get all inventory items that are children of the supplied ItemClass. */
	UFUNCTION(BlueprintPure, Category = "Inventory")
	TArray<UOItem*> FindItemsOfClass(TSubclassOf<class UOItem> ItemClass, const bool bCheckVisibility = false) const;
	...
	UFUNCTION(BlueprintPure, Category = "Inventory")
	virtual int32 GetSpaceForItem(TSubclassOf<class UOItem> ItemClass, FText& NoSpaceReason) const;
	...
	UFUNCTION(BlueprintPure, Category = "Inventory")
	FORCEINLINE TArray<class UOItem*> GetItems() const { return Items; }
...
protected:
	...
	virtual FItemAddResult TryAddItem_Internal(TSubclassOf<class UOItem> ItemClass, const int32 Quantity = 1);
...
public:
	...
	UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly, Category = "Inventory")
	virtual void SetLootSource(class UOInventoryComponent* NewLootSource);

protected:

	UFUNCTION(Server, Reliable)
	virtual void ServerRemoveItem(class UOItem* Item);

	UFUNCTION(Server, Reliable)
	virtual void ServerConsumeItem(class UOItem* Item, const int32 Quantity);

	UFUNCTION(Server, Reliable)
	virtual void ServerUseItem(class UOItem* Item);
	...
	UFUNCTION(BlueprintCallable, Category = "Inventory")
	virtual bool RequestLootItem(class UOItem* ItemToLoot, FText& ErrorText, const int32 Quantity = 1);

	UFUNCTION(Server, Reliable)
	virtual void ServerRequestLootItem(class UOItem* ItemToLoot, const int32 Quantity);
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Inventory")
	virtual bool RequestStoreItem(class UOItem* ItemToStore, FText& ErrorText, const int32 Quantity = 1);

	UFUNCTION(Server, Reliable)
	virtual void ServerRequestStoreItem(class UOItem* ItemToLoot, const int32 Quantity);
	...
	*/
	...
	int32 GetBuyPrice(TSubclassOf<class UOItem> Item, int32 Quantity = 1) const;
	virtual int32 GetBuyPrice_Implementation(TSubclassOf<class UOItem> Item, int32 Quantity = 1) const;

	/**
	* Return the price we'll sell the given item for
	*/
	UFUNCTION(BlueprintNativeEvent, BlueprintPure, Category = "Item")
	int32 GetSellPrice(TSubclassOf<class UOItem> Item, int32 Quantity = 1) const;
	virtual int32 GetSellPrice_Implementation(TSubclassOf<class UOItem> Item, int32 Quantity = 1) const;

private:
	
	/**Don't call Items.Add() directly, use this function instead, as it handles replication and ownership*/
	UOItem* AddItem(TSubclassOf<class UOItem> ItemClass, const int32 Quantity);

	UFUNCTION()
	void OnRep_Items(const TArray<class UOItem*>& OldItems);
	
	UFUNCTION()
	void OnRep_Currency(const int32 OldCurrency);

	UFUNCTION()
	void OnRep_LootSource(class UOInventoryComponent* OldLootSource);
