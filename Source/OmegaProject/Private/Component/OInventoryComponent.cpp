...
UOInventoryComponent::UOInventoryComponent(const FObjectInitializer& ObjectInitializer): Super(ObjectInitializer)
{
...
bool UOInventoryComponent::UseItem(UOItem* Item)
{
	if (!IsValid(Item))
	{
		return false;
...
void UOInventoryComponent::TryAddFromLootTable(FLootTableRoll LootTable, TArray<FItemAddResult>& OutItemAddResults)
{
...
FItemAddResult UOInventoryComponent::TryAddItemFromClass(TSubclassOf<class UOItem> ItemClass, const int32 Quantity /*=1*/, const bool bCheckAutoUse/*=false*/)
{
...
int32 UOInventoryComponent::ConsumeItem(class UOItem* Item, const int32 Quantity)
{
...
bool UOInventoryComponent::RemoveItem(class UOItem* Item)
{
...
bool UOInventoryComponent::HasItem(TSoftClassPtr<class UOItem> ItemClass, const int32 Quantity /*= 1*/, const bool bCheckVisibility/*= false*/) const
{
	if (!ItemClass.IsNull())
	{	
		//Todo re-write this, it needs to iterate the inventory and find all items with quantity 
		if (UOItem* ItemToFind = FindItemByClass(ItemClass))
		{
			return true;
		}
		else
...
bool UOInventoryComponent::AllowLootItem(class UOInventoryComponent* Taker, TSubclassOf<class UOItem> ItemClass, const int32 Quantity, FText& ErrorText)  const
{
	...
	if (Taker)
	{
		...
		const bool bShopHasItem = HasItem(TSoftClassPtr<UOItem>(ItemClass), Quantity);
...
bool UOInventoryComponent::AllowStoreItem(class UOInventoryComponent* Storer, TSubclassOf <class UOItem> ItemClass, const int32 Quantity, FText& ErrorText) const
{
	if (Storer)
	{
		const bool bWeHaveSpace = GetSpaceForItem(ItemClass, ErrorText) >= Quantity;
		const bool bStorerHasItem = Storer->HasItem(TSoftClassPtr<UOItem>(ItemClass), Quantity);
...
FItemAddResult UOInventoryComponent::PerformLootItem(class UOInventoryComponent* Taker, TSubclassOf <class UOItem> ItemClass, const int32 Quantity /*= 1*/)
{
	if (Taker)
	{
		...
		if (AddResult.AmountGiven > 0)
		{
			if (UOItem* Item = FindItemByClass(TSoftClassPtr<UOItem>(ItemClass)))
			{
...
FItemAddResult UOInventoryComponent::PerformStoreItem(class UOInventoryComponent* Storer, TSubclassOf <class UOItem> ItemClass, const int32 Quantity /*= 1*/)
{
	if (Storer)
	{
		...
		if (AddResult.AmountGiven > 0)
		{
			if (UOItem* Item = Storer->FindItemByClass(TSoftClassPtr<UOItem>(ItemClass)))
			{
...
UOItem* UOInventoryComponent::FindItemByClass(TSoftClassPtr<class UOItem> ItemClass, const bool bCheckVisibility/*= false*/) const
{
...
TArray<UOItem*> UOInventoryComponent::FindItemsOfClass(TSubclassOf<class UOItem> ItemClass, const bool bCheckVisibility/*= false*/) const
{
	TArray<UOItem*> ItemsOfClass;
...
int32 UOInventoryComponent::GetSpaceForItem(TSubclassOf<class UOItem> ItemClass, FText& NoSpaceReason) const 
{
	...
	if (const UOItem* ItemCDO = GetDefault<UOItem>(ItemClass))
	{
		...
		if (UOItem* ExistingItem = FindItemByClass(TSoftClassPtr<UOItem>(ItemClass)))
		{
...
int32 UOInventoryComponent::GetCurrency() const
{
	return Currency;
}

void UOInventoryComponent::AddCurrency(const int32 Amount)
{
...
void UOInventoryComponent::SetCurrency(const int32 Amount)
{
	if (GetOwnerRole() >= ROLE_Authority && Amount >= 0)
	{
...
float UOInventoryComponent::GetCurrentWeight() const
{
	float Weight = 0.f;
...
void UOInventoryComponent::SetWeightCapacity(const float NewWeightCapacity)
{
	WeightCapacity = NewWeightCapacity;
	OnInventoryUpdated.Broadcast();
}

void UOInventoryComponent::SetCapacity(const int32 NewCapacity)
{
	Capacity = NewCapacity;
	OnInventoryUpdated.Broadcast();
}

void UOInventoryComponent::SetInventoryFriendlyName(const FText& Name)
{
	InventoryFriendlyName = Name;
}

void UOInventoryComponent::SetIsVendor(const bool bNewIsVendor)
{
	bIsVendor = bNewIsVendor;
}

APawn* UOInventoryComponent::GetOwningPawn() const
{
	if (OwnerPC)
	{
...
APlayerController* UOInventoryComponent::GetOwningController() const
{
...
void UOInventoryComponent::ClientRefreshInventory_Implementation()
{
	OnInventoryUpdated.Broadcast();
}

//bool UOInventoryComponent::Save(const FString& SaveName /*= "NarrativeInventorySaveData"*/, const int32 Slot /*= 0*/)
/*{
...
//bool UOInventoryComponent::Load(const FString& SaveName /*= "InventorySaveData"*///, const int32 Slot /*= 0*/)
/*{
	...
	if (UInventorySaveGame* InventorySave = Cast<UInventorySaveGame>(UGameplayStatics::LoadGameFromSlot(SaveName, Slot)))
	{
		...
		for (auto& InventoryItem : InventorySave->SavedItems)
		{
			if (IsValid(InventoryItem.ItemClass) && InventoryItem.Quantity > 0)
			{
				...
				if (UOItem* AddedItem = AddResult.Stacks[0])
				{
...
//bool UOInventoryComponent::DeleteSave(const FString& SaveName /*= "InventorySaveData"*///, const int32 Slot /*= 0*/)
/*{
...
*/
void UOInventoryComponent::BeginPlay()
{
	Super::BeginPlay();
...
void UOInventoryComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(UOInventoryComponent, Items);
	DOREPLIFETIME(UOInventoryComponent, Currency);

	//If your game requires players to know if other players are looting you can remove 
	DOREPLIFETIME_CONDITION(UOInventoryComponent, LootSource, COND_OwnerOnly);
	
}

bool UOInventoryComponent::ReplicateSubobjects(class UActorChannel* Channel, class FOutBunch* Bunch, FReplicationFlags* RepFlags)
{
...
UOItem* UOInventoryComponent::AddItem(TSubclassOf<class UOItem> ItemClass, const int32 Quantity)
{
	if (Quantity > 0 && GetOwner() && GetOwner()->HasAuthority())
	{
		...
		UOItem* NewItem = NewObject<UOItem>(GetOwner(), ItemClass);
...
void UOInventoryComponent::OnRep_Items(const TArray<class UOItem*>& OldItems)
{
	OnInventoryUpdated.Broadcast();
...
void UOInventoryComponent::OnRep_Currency(const int32 OldCurrency)
{
	OnCurrencyChanged.Broadcast(OldCurrency, Currency);
}

void UOInventoryComponent::OnRep_LootSource(class UOInventoryComponent* OldLootSource)
{
	if (LootSource)
	{
...
FItemAddResult UOInventoryComponent::TryAddItem_Internal(TSubclassOf<class UOItem> ItemClass, const int32 Quantity /*= 1*/)
{
	if (GetOwner() && GetOwner()->HasAuthority() && IsValid(ItemClass))
	{
		if (const UOItem* ItemCDO = GetDefault<UOItem>(ItemClass))
		{
			...
			{
				if (UOItem* NewItem = AddItem(ItemClass, MaxStackSize))
				{
			...
			if (Remainder > 0)
			{
				if (UOItem* NewItem = AddItem(ItemClass, Remainder))
				{
					AddResult.Stacks.Add(NewItem);
				}
			}
...
void UOInventoryComponent::GiveDefaultItems()
{
...
void UOInventoryComponent::ServerRemoveItem_Implementation(class UOItem* Item)
{
	if (Item)
	{
		RemoveItem(Item);
...
void UOInventoryComponent::ServerConsumeItem_Implementation(class UOItem* Item, const int32 Quantity)
{
	if (Item)
	{
...
void UOInventoryComponent::ServerUseItem_Implementation(class UOItem* Item)
{
	UseItem(Item);
}

void UOInventoryComponent::ServerStopLooting_Implementation()
{
	StopLooting();
}

void UOInventoryComponent::StopLooting()
{
	if (GetOwnerRole() < ROLE_Authority)
	{
		ServerStopLooting();
...
bool UOInventoryComponent::RequestLootItem(class UOItem* ItemToLoot, FText& ErrorText, const int32 Quantity /*= 1*/)
{
...
void UOInventoryComponent::ServerRequestLootItem_Implementation(class UOItem* ItemToLoot, const int32 Quantity)
{
	FText DummyText;
	RequestLootItem(ItemToLoot, DummyText, Quantity);
}

bool UOInventoryComponent::RequestStoreItem(class UOItem* ItemToStore, FText& ErrorText, const int32 Quantity /*= 1*/)
{
...
void UOInventoryComponent::ServerRequestStoreItem_Implementation(class UOItem* ItemToLoot, const int32 Quantity)
{
	FText DummyText;
	RequestStoreItem(ItemToLoot, DummyText, Quantity);
}

int32 UOInventoryComponent::GetBuyPrice_Implementation(TSubclassOf<class UOItem> Item, int32 Quantity /*= 1*/) const
{
	if (IsValid(Item))
	{
		if (const UOItem* ItemCDO = GetDefault<UOItem>(Item))
		{
...
int32 UOInventoryComponent::GetSellPrice_Implementation(TSubclassOf<class UOItem> Item, int32 Quantity /*= 1*/) const
{
	if (IsValid(Item))
	{
		if (const UOItem* ItemCDO = GetDefault<UOItem>(Item))
...
void UOInventoryComponent::SetLootSource(class UOInventoryComponent* NewLootSource)
{
	if (LootSource != NewLootSource)
	{
		UOInventoryComponent* OldSource = LootSource;
